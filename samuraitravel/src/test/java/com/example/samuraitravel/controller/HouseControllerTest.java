...
public class HouseControllerTest {
	...
	public void ログイン済みの場合は会員用の民宿一覧ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/houses")).andExpect(status().isOk()).andExpect(view().name("houses/index"));
	}

	@Test
	public void 未ログインの場合は会員用の民宿詳細ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/houses/1")).andExpect(status().isOk()).andExpect(view().name("houses/show"));
	}
